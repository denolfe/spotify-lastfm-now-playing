...
GetSongInfo:
	...
	{
		Url_Artist := "http://ws.audioscrobbler.com/2.0/?method=artist.gettopalbums&artist=" Artist "&api_key=" api_key "&format=json"
		URLDownloadToFile, % Url_Artist , % artist_json_file
		FileRead, a, % artist_json_file
		album_art_url := UnJson(json(a, "topalbums.album[0].image[3].#text"))
		FileCopy, img\Unknown.png, % album_art, 1
		source := "Source: Top Artist Albums"
		; Notify(Artist . " - " . Track, "Album Art Not Found",-4,"Style=Fast Image=img\no-cover.png")
	}
	Else
	{
