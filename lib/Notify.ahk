...
Notify(Title="Notify()",Message="",Duration="",Options="")
{
    ...
    If (Options)     ; skip parsing steps if Options param isn't used
    {
        ...
        {     ; so "Style=Default GC=Blue" is allowed, which will reset all defaults and then set GC=Blue
            ...
            Else If Style = Alert
                Return % Notify(Title,Message,Duration,"SI=50 GC=Red TC=White MC=White SI=350 ST=450 BC=White GR=9 BR=13 BW=0 BT=105 TS=9 MS=8 " Options "Style=")
            Else If Style = Fast
                Return % Notify(Title,Message,Duration,"SI=50 GC=555555 TC=White MC=White SI=100 ST=100 BC=00000 GR=9 BR=13 BW=0 BT=105 TS=9 MS=8 " Options "Style=")
        }
    }
    ;—————— end if options ————————————————————————————————————————————————————————————————————————————
